...
import { ChronoGraphI, ChronoRevision, PropagationResult, RevisionNode } from "./Graph.js"
...
class ChronoAtom extends base {
    ...
    maxRevisionAvailable    : ChronoRevision

    quarksMap               : Map<RevisionNode, Quark>      = new Map()

    cachedCurrentQuark      : Quark
    cachedCurrentRevision   : RevisionNode


    getCurrentQuarkMap () : Quark {
        const snapshotRevision      = this.graph.currentRevision
        //
        // if (this.maxRevisionAvailable < snapshotRevision
        //
        // if (this.snapshot.revision === this.cachedCurrentRevision) return this.cachedCurrentQuark
        //
        // this.cachedCurrentRevision = this.graph.currentRevision
        //
        return this.cachedCurrentQuark = this.quarksMap.get(this.cachedCurrentRevision)
    }
