...
import { NotChanged } from "../../src/chrono/Effect.js";
StartTest(t => {
    t.it('Atom as quark revisions graph', async (t) => {
        const atom1 = MinimalChronoAtom.new({
        ...
        const atom2 = MinimalChronoAtom.new({ value: 1 });
        const atom3 = MinimalChronoAtom.new({
            calculation: function* () {
                return (yield atom1) + (yield atom2);
            }
        });
        atom1.put(0);
        t.is(atom1.hasValue(), false, 'Atom has no value');
        t.is(atom2.hasValue(), false, 'Atom has no value');
    ...
    // t.it('Atom outside of graph', async t => {
    //     const graph : ChronoGraph   = MinimalChronoGraph.new()
    //
    //     const atom1 : ChronoAtom    = MinimalChronoAtom.new({
    //         calculation   : function * () {
    //             const input     = yield atom1.input
    //
    ...
    //         }
    //     })
    //     const atom2 : ChronoAtom    = MinimalChronoAtom.new({ value : 1 })
    //
    //     const atom3 : ChronoAtom    = MinimalChronoAtom.new({
    //         calculation     : function * () {
    //             return (yield atom1) + (yield atom2)
    //         }
    //     })
    ...
    //     t.is(atom3.hasValue(), false, 'Atom has no value')
    //
    //     graph.addNodes([ atom1, atom2, atom3 ])
    //
    //     graph.propagateSync()
    //
    //     t.is(atom3.hasValue(), true, "Correct result calculated")
