...
StartTest(t => {
    t.it('Minimal mutation in graph context', t => {
        ...
        const box1 = graph.addBox(MinimalBox.new({ id: 1 }));
        const box2 = graph.getBox(2);
        const box3 = graph.getBox(3);
        box1.set(0);
        box2.set(1);
        graph.compute(box3, () => {
    ...
    t.it('2 mutations in graph context', t => {
        const graph = MinimalGraphBox.new({ id: 'graph' });
        const box1 = graph.getBox('inp1');
        const box2 = graph.getBox('inp2');
        const box1p2 = graph.getBox('1p2');
        const box3 = graph.getBox('inp3');
        const res = graph.getBox('res');
